...
RAW_DATA_DIR=${CURRENT_DIR}/raw_data/figure_12
PY_DIR=${CURRENT_DIR}/python/figure_12
RESULT_DIR=${CURRENT_DIR}/results/figure_12
...
unset OVERSUBSCRIPTION_FACTOR
unset PYTORCH_CUDA_ALLOC_CONF
export OVERSUBSCRIPTION_FACTOR=3.0
export PYTORCH_CUDA_ALLOC_CONF=use_uvm:True

echo "OVERSUBSCRIPTION_FACTOR=${OVERSUBSCRIPTION_FACTOR}" >> ${LOG_FILE}
echo "PYTORCH_CUDA_ALLOC_CONF=${PYTORCH_CUDA_ALLOC_CONF}" >> ${LOG_FILE}
...
cd ${UVM_ADVISOR_PATH}
make -j
cd ${UVM_ADVISOR_PATH}/uvm_helper
make -j

# no prefetch
echo "" >> ${LOG_FILE}
echo "-------------------------------- NO PREFETCH --------------------------------" >> ${LOG_FILE}
prefix_command="PREFETCH_MODE=0 LD_PRELOAD=${UVM_ADVISOR_PATH}/libop_callback_uvm.so"
